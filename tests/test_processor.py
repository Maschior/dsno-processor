...
def mock_processor_config(monkeypatch):
    from dsno_processor.config import AppConfig
    import dsno_processor.processor
    monkeypatch.setattr(dsno_processor.processor, "_safe_config", lambda: AppConfig())
...
class TestProcessDsno:
    ...
    def test_keep_original(
        ...
        from dsno_processor.config import AppConfig, ProcessorConfig
        cfg = AppConfig(processor=ProcessorConfig(keep_original=True))
        import dsno_processor.processor
        monkeypatch.setattr(dsno_processor.processor, "_safe_config", lambda: cfg)

        mock_read_ctrl.return_value = MagicMock()
        mock_pairs.return_value = [(100001, "DSNO_001.txt", 100, 110)]
        mock_single.return_value = None

        # Create a dummy original DSNO file so the check exists doesn't fail
        dsno_file = tmp_path / "DSNO_001.txt"
        dsno_file.write_text("dummy")
        ...
        mock_move_shutil.assert_called_once_with(
            str(dsno_file),
            str(tmp_path / "Processed" / "original_files" / "DSNO_001.txt"),
        )
