...
def process_dsno(
    ...
    for invoice, dsno_filename, oracle_freight, softway_freight in pairs:
        ...
        if cfg.processor.keep_original:
            ...
            target_dsno_path = processed_dir / dsno_filename
            try:
                shutil.copy2(dsno_path, target_dsno_path)
            except Exception as exc:
                error = f"Failed to copy file for processing: {exc}"
                result.errors.append(error)
                log.error(error)
                _cb("error", {"name": dsno_path.name, "detail": error})
                continue
        ...
        if error:
            ...
            if cfg.processor.keep_original:
                ...
                try:
                    shutil.move(str(dsno_path), str(original_files_dir / dsno_filename))
                    log.info("Original DSNO file moved to: %s", original_files_dir / dsno_filename)
                except Exception as exc:
                    log.error("Failed to move original DSNO file %s: %s", dsno_path.name, exc)
            else:
                move_to_processed(dsno_path, processed_dir)
            _cb("success", {"name": dsno_path.name})

    # Update status — always update both spreadsheet and database
    _cb("phase", {"text": "Updating control sheet..."})
    try:
        update_control_sheet_status(control_sheet, processed_dir)
    except Exception as exc:
        log.warning("Could not update control sheet statuses: %s", exc)
