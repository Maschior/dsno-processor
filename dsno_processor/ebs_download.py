...
from .i18n import t
from .database import get_connection, get_db_path, update_statuses_for_downloaded
from .status_updater import update_excel_status_for_downloaded
...
def run_download(
    ...
    try:
        ...
        # 8. Update database statuses for successfully downloaded files
        if success_count > 0:
            try:
                db_path = get_db_path()
                if db_path.exists():
                    downloaded_set = {
                        ...
                        if already_downloaded(history, fn)
                    }
                    if downloaded_set:
                        conn = get_connection(db_path)
                        db_updated = update_statuses_for_downloaded(
                            conn, downloaded_set
                        )
                        conn.close()
                        if db_updated:
                            logger.info(
                                "Updated %d status(es) to 'Downloaded' in database.",
                                db_updated,
                            )
            except Exception as exc:
                logger.warning("Could not update database statuses: %s", exc)

            # Update spreadsheet statuses always
            try:
                sheet_updated = update_excel_status_for_downloaded(
                    config.customer_sheet_path,
                    config.download_dir,
                    dsno_col=config.dsno_col,
                    status_col=config.status_col,
                )
                if sheet_updated:
                    logger.info(
                        "Updated %d status(es) to 'Downloaded' in spreadsheet.",
                        sheet_updated,
                    )
            except Exception as exc:
                logger.warning(
                    "Could not update spreadsheet statuses: %s", exc
                )
