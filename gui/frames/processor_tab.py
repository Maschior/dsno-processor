...
class ProcessorTabMixin:
    ...
    def _build_tab_processor(
        ...
        self.control_row = FilePickerRow(
            ...
            on_change=self._on_control_sheet_changed,
        )
        ...
        ctk.CTkButton(
            ...
            text_color="white",
            font=ctk.CTkFont(family=_FONT_FAMILY, size=11),
            command=self._sync_spreadsheet_status,
    ...
    def _update_run_btn_state(self) -> None:
        ...
        try:
            cfg = load_config()
            is_db = getattr(cfg.general, "data_source", "spreadsheet") == "database"
        except Exception:
            is_db = False
    ...
    def _on_control_sheet_changed(self) -> None:
        """Handle control sheet value changes."""
        self._update_run_btn_state()
        self.refresh_status_options()

    def _update_customer_row_visibility(self) -> None:
        """Show or hide the customer sheet row based on the configured data source."""
        try:
            cfg = load_config()
            is_db = getattr(cfg.general, "data_source", "spreadsheet") == "database"
        except Exception:
        ...
        if is_db:
            if hasattr(self, "customer_row"):
                self.customer_row.pack_forget()
        else:
            if hasattr(self, "customer_row") and hasattr(self, "control_row"):
                self.customer_row.pack(fill="x", pady=4, before=self.control_row)

    def refresh_status_options(self) -> None:
        """Dynamically reload options in the status filter dropdown."""
        import os
        try:
            cfg = load_config()
            if getattr(cfg.general, "data_source", "spreadsheet") == "database":
                ...
                control_path = self.control_row.get().strip() if hasattr(self, "control_row") else ""
                if control_path and os.path.exists(control_path):
                    sheet_df = read_control_sheet(control_path)
                    status_options = get_status_options(sheet_df)
                else:
                    status_options = []
        except Exception as exc:
            log.warning("Could not refresh status options: %s", exc)
            status_options = []

        if hasattr(self, "filter_by_status"):
            self.filter_by_status.set_options(status_options)
