...
class ImportControlWizard(ctk.CTkToplevel):
    ...
    def _import_thread(self, file_path: str) -> None:
        try:
            ...
            imported, updated, skipped = db_import_control_sheet(
                conn,
                file_path,
                invoice_col=inv_col,
                dsno_col=dsno_col,
            ...
            msg = t("import_control.success", imported=imported, updated=updated, skipped=skipped)
            self.after(
                0, lambda: self._status_label.configure(text=msg, text_color="#43a047")
            )
            self.after(0, lambda: messagebox.showinfo(t("import_control.title"), msg))
            if hasattr(self.master, "refresh_status_options"):
                self.after(0, self.master.refresh_status_options)
        except Exception as exc:
