"""Bulk import control sheets into the local SQLite database.

Recursively scans a directory (and all subdirectories) for Excel files
matching the control sheet pattern and imports their data into
``tb_control``.

A JSON history file (``import_control_history.json``) is kept next to the database
...
# Control sheet column defaults are handled by argparse

_EXCEL_GLOBS = ("*.xlsx", "*.xls")

_HISTORY_FILENAME = "import_control_history.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Bulk import control sheets into the DSNO Processor database.",
    )
    parser.add_argument(
        "directory",
        type=str,
        help="Root directory to scan for control sheet Excel files.",
    )
    parser.add_argument(
        "--invoice-col",
        default=None,
        help="Column name for invoice number (defaults to config.toml).",
    )
    parser.add_argument(
        "--dsno-col",
        default=None,
        help="Column name for DSNO filename (defaults to config.toml).",
    )
    parser.add_argument(
        "--date-col",
        default=None,
        help="Column name for creation date (defaults to config.toml).",
    )
    parser.add_argument(
        "--status-col",
        default=None,
        help="Column name for status (defaults to config.toml).",
    )
    parser.add_argument(
        "--oracle-col",
        default=None,
        help="Column name for Oracle freight (defaults to config.toml).",
    )
    parser.add_argument(
        "--softway-col",
        default=None,
    ...
    for i, filepath in enumerate(pending, 1):
        ...
        try:
            imported, skipped = import_control_sheet(
                conn,
                filepath,
                invoice_col=args.invoice_col,
                dsno_col=args.dsno_col,
